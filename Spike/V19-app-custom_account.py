# V-19 probe payload.  Copied from zelin
# (Reference/zelin-tech-erpnext_china/erpnext_china/chart_of_accounts/
# custom_accounts/custom_account.py, MIT) with ONLY the on-disk app name changed
# erpnext_china -> erx_v19 in the two custom chart paths.
#
# Unlike the V-18 payload, this one DOES include `erpnext_china_create_charts`
# and its three helpers -- that function is exactly what option A relies on and
# what V-18 deliberately left out.
#
# Note the shadowing that is present in zelin's original and preserved here:
# `add_suffix_if_duplicate` and `identify_is_group` are imported from erpnext at
# the top and then re-defined at module level further down, so the local
# definitions are the ones `_import_accounts` actually calls.
...
def get_chart(chart_template, existing_company=None):
    ...
    if existing_company:
        ...
        custom_path = os.path.join(bench_dir, "apps", "erx_v19", "erx_v19", "chart_of_accounts", "custom_accounts")
